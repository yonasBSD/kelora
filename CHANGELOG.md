...
The **2.0** line. The headline changes are a redesigned tracking-function family, a set of named application-log formats, composable parser cascades, and a much more capable `--discover` mode. Breaking changes are flagged below — most affect tracking scripts and a few error/validation behaviors. See [What's New in 2.0](docs/whats-new-2.0.md) for migration guidance with old → new examples.
