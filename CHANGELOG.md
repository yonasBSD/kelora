...
- **Human metrics table gets labeled, aligned columns** - In the `--metrics=full`/human table, list-valued metrics (frequency tables and `track_top`/`track_bottom[_by]` rankings) now print a two-column table with a header row naming each column and the numbers right-aligned underneath. The number column is named for what it holds — `count` for occurrence tallies (`track_freq`/`track_top`/`track_bottom`) and `score` for value rankings (`track_top_by`/`track_bottom_by`) — so a row like `200   3` reads unambiguously as a `value` of `200` with a `count` of `3`, rather than two adjacent numbers. The redundant `#1`/`#2` rank prefix is dropped: rows are already emitted in rank order, so position conveys rank. Only the human table's layout changed; the stored numbers, the `tsv` stream, and the JSON output are untouched, as are scalar metrics (`= …` for sums/averages, `≈ …` for cardinality).
