...
- **`--discover` marks the timestamp field** - Field discovery now identifies the primary timestamp field — the one kelora would use for `--since`/`--until` — and marks its row with `⏱` (`(ts)` under `--no-emoji`). The footer gains a `timestamp:` fragment beside the existing `format:` one (e.g. `timestamp: ts`, `timestamp: ts (60% parsed)` when some values don't parse, or `timestamp: when (--ts-field)` for an override); it stays quiet when no timestamp field is found. JSON output (`-d=json`) gains a top-level `timestamp` object (`field`, `source`, `detected`, `parsed`) and a `"timestamp": true` flag on the field. This answers "which field is the clock?" at the moment of orienting to an unknown file — before a time filter silently drops everything.
