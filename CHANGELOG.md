...
- **Breaking: tracking-function redesign (`track_freq`, `track_inc`, `track_top`/`track_bottom`)** - The tracking family is consolidated around one convention: `track_fn(name, categorical-or-numeric args...)`.
    ...
    - All `track_*` functions now skip Unit `()` values (missing fields) instead of erroring — previously the categorical counter failed on every event when the field was absent. Skips are counted per metric, and a `--diagnostics` hint surfaces a likely field-name typo. The hint is gated to fire only when a metric recorded a value on *no* event (the field was missing from every event), and only once the input clears a small minimum-sample threshold — so a field that is simply present in some events and absent in others (normal for varying-shape logs) no longer triggers it.
