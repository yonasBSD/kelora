...
- **`postgres` named format** - PostgreSQL server logs join the named application-log catalogue. `postgres` parses logs written with the default `log_line_prefix = '%m [%p] '` — `2024-01-02 15:04:05.123 UTC [1234] LOG:  database system is ready` — into `ts`, `log_tz`, `pid`, `level`, and `msg`; a customized prefix (user@db, application name, …) won't auto-detect, so reach those with `-f regex:`. Its `ts` is naive and resolved through `--input-tz` (default UTC) like the other timestamp-only formats — the logged zone abbreviation is kept in `log_tz` (named so it reads as "what the server logged", not "the zone applied") for inspection but not applied, since abbreviations are ambiguous and can't encode an offset/DST; UTC-logged servers (the common config) are correct by default, and a non-UTC server is handled with `--input-tz <IANA>`. Multi-line errors (an `ERROR:`/`STATEMENT:` record followed by tab-indented query-continuation lines) parse cleanly with `-M indent`, which folds the indented lines into the preceding record; `-f postgres,line` instead keeps unmatched continuation lines as raw `line` events. Selectable with `-f <name>`, usable in cascades, and shown in `--help-formats`. (Adapted from [lnav](https://lnav.org), BSD-3-Clause.)
...
- **Datetime/duration values stored in a field render correctly in output instead of leaking an internal type name** - The companion to the script-interpolation fix above: assigning a `DateTimeWrapper`/`DurationWrapper` to an event field and emitting it — e.g. `--exec 'e.t = meta.parsed_ts'` (the very field the `--discover` footer advertises) or `e.d = duration_from_seconds(90)` — printed the Rust type path (`kelora::rhai_functions::datetime::DateTimeWrapper`) in `default`, `logfmt`, `json`, `csv`, and `inspect` output, and an unquoted token in metrics/`to_json` serialization. The output serializers fell through to Rust-level `Dynamic::to_string()`/`{:?}`, which returns the type name for custom types (the registered Rhai `to_string` only covers script-level use). All serialization paths now route these two wrappers through their `Display`, so a datetime renders as `2026-01-02T15:04:05+00:00` and a duration as `1m 30s` (quoted where a format needs it), and `--discover` reports their type as `datetime`/`duration`. The 1-argument `to_json(map)` form still defers to Rhai's built-in `Map::to_json()` (use the explicit `to_json(value, indent)` overload for wrapper-aware output).
