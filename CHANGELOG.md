...
- **`--discover` reports the timestamp field** - Field discovery now identifies the primary timestamp field — the one kelora would use for `--since`/`--until` — and reports it in the footer beside the existing `format:` fragment (e.g. `timestamp: ts`, `timestamp: ts (60% parsed)` when some values don't parse, `timestamp: when (--ts-field)` for an override, or `timestamp: when (--ts-field, not found)` when the override names a missing field); it stays quiet when no timestamp field is found. JSON output (`-d=json`) gains a top-level `timestamp` object (`field`, `source`, `detected`, `parsed`) and a `"timestamp": true` flag on the field. The table rows themselves carry no marker, so column-based parsing of the output is unaffected. This answers "which field is the clock?" at the moment of orienting to an unknown file — before a time filter silently drops everything.
...
- **Breaking: tracking-function redesign (`track_count`, `track_bucket`, `track_top`/`track_bottom`)** - The tracking family is consolidated around one convention: `track_fn(name, categorical-or-numeric args...)`.
    ...
    - Reusing one metric name across different track functions (e.g. `track_sum("x", ...)` then `track_min("x", ...)`) is now a call-time error. Previously the conflicting values were silently blended into garbage during parallel merging.
    - Fixed: a `track_count` category map whose categories are literally named `sum` and `count` no longer renders as a bogus average — output finalization now keys off the recorded operation instead of sniffing the value's shape.
    - `track_unique` warns once past 100,000 stored values (it keeps every distinct value in memory by design); the warning points to `track_cardinality()` for unique counts, honors `--silent`/`--no-diagnostics`, and also fires in `--parallel` (where the full set only exists after the merge).
    ...
    - In `--parallel` runs the conflict check cannot see across threads, so a metric name used by different track functions in `--begin` vs the event stages is reported as a merge-time warning ("the merged value is unreliable") rather than a per-call error. Known limitation: `track_stats` suffix keys (`{name}_min`, `{name}_sum`, ...) share aggregation ids with the standalone functions and can silently blend with them — keep `track_stats` base names distinct.
    - Metric names starting with `__kelora_` or `__op_` are reserved for internal bookkeeping and are hidden from metrics output (both text and JSON).
...
- **`line`-fallback hint points at field extraction** - When auto-detection finds no structured format and keeps whole lines as `line`, the hint now suggests extracting fields from `timestamp LEVEL message` app logs with `-f 'cols:ts(2) level *msg'` (or a `regex:`), and cascading a mixed file with repeated `-f` — instead of only mentioning `-j`/`-f <fmt>`.
