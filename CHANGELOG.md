...
- **Named application-log formats** - A small curated set of common application-log layouts — `glog` (Go/klog), `nginx-error`, `log4j`/Java, `python-logging`, and `iso8601-level` (generic ISO-8601 timestamp + level) — are now recognised and parsed into structured fields via the regex engine. They are first-class: selectable with `-f <name>` (e.g. `-f log4j`), usable inside cascade lists (`-f log4j,line`), shown by name in the auto-detect notice and `--stats` ("Detected format: log4j"), and documented in `--help-formats`. During auto-detection they are tried only as the last step before the `line` fallback, so no format Kelora already detected changes. The definitions are adapted from [lnav](https://lnav.org) (BSD-3-Clause; see `THIRD_PARTY_LICENSES.md`).
