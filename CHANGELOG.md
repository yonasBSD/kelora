...
- **`-k/--keys` explains rows silently dropped by key selection** - Selecting fields that are present in *some* rows but not others (`kelora -k user,action audit.log` on heterogeneous logs) silently dropped every row that lacked all of them: the row became empty after key selection and was removed, surfacing only as an unexplained bump in the "filtered" count even though no `--filter`/`--levels`/time gate ran. Kelora now reports it ("-k/--keys dropped 3 events that had none of the selected fields (user, action)…") and points at `-d/--discover` to see how often each field is missing; `--exclude-keys` that strips a row's every field gets a parallel message. The hint fires only when rows were actually emptied this way, defers to the existing "never present" typo hint when a requested key was seen nowhere (so it never doubles up on a plain typo), goes to stderr, keeps exit code `0`, and is suppressed by `--no-diagnostics`/`-q`/`--silent`.
