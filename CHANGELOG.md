...
- **Human metrics table labels its number column (`by count` vs `by score`)** - In the `--metrics=full`/human table, ranked and frequency metrics now state what their trailing number means in the header: `track_freq`/`track_top`/`track_bottom` read `(N items, by count)` and the score-ranked `track_top_by`/`track_bottom_by` read `(N items, by score)`. Previously every list rendered an identical bare number column, so a score value (`track_top_by("bytes", e.url, e.bytes)` → `300`) was easily misread as an occurrence count. The numbers themselves, the `tsv` stream, and the JSON output are unchanged — only the table's header gained the annotation; scalars (`= …` for sums/averages, `≈ …` for cardinality) were already unambiguous and are untouched.
