...
### Internal Benchmarks (Throughput / Regression Testing)
...
These targets run the throughput suite (`bench_simple_cases.py`), described in
detail under [Throughput Suite](#throughput-suite) below.
...
`bench_simple_cases.py` (run via `just bench`) targets the **per-event hot
path** for the cases real users hit most often (parse → filter → format →
write). It reports **throughput** (lines/s and MB/s) rather than raw wall-time,
so results are comparable across machines and dataset sizes.
