...
impl RhaiEngine {
    ...
    fn register_functions(engine: &mut Engine) {
        ...
        engine.register_fn("cols", |text: &str, selectors: rhai::Array, sep: &str| -> rhai::Array {
            selectors.into_iter()
                .filter_map(|s| s.into_string().ok())
                .map(|selector| Dynamic::from(Self::extract_columns(text, &selector, sep, " ")))
                .collect()
        });
        ...
        engine.register_fn("cols", |text: &str, selectors: rhai::Array, sep: &str, outsep: &str| -> rhai::Array {
            selectors.into_iter()
                .filter_map(|s| s.into_string().ok())
                .map(|selector| Dynamic::from(Self::extract_columns(text, &selector, sep, outsep)))
                .collect()
        });

        // Note: derive() function requires scope access and will be implemented differently
        // For now, users can use cols() and assign manually
    ...
    fn extract_columns(text: &str, selector: &str, sep: &str, outsep: &str) -> String {
        let columns = Self::split_text(text, sep);
        let indices = Self::parse_selector(selector, columns.len());
    ...
    fn parse_selector(selector: &str, total_cols: usize) -> Vec<usize> {
        ...
        for part in selector.split(',') {
            ...
            if part.contains(':') {
                ...
                let range_indices = Self::parse_range_selector(part, total_cols);
                indices.extend(range_indices);
            } else {
                // Single index selector
                if let Ok(idx) = part.parse::<i32>() {
                    if let Some(pos) = Self::normalize_index(idx, total_cols) {
                        indices.push(pos);
                    }
                }
            }
    ...
    fn parse_range_selector(range: &str, total_cols: usize) -> Vec<usize> {
        ...
        let start = if parts[0].trim().is_empty() {
            ...
            match parts[0].trim().parse::<i32>() {
                Ok(idx) => match Self::normalize_index(idx, total_cols) {
...
mod tests {
    ...
    fn test_extract_columns_basic() {
        assert_eq!(RhaiEngine::extract_columns("alpha beta gamma", "0", " ", " "), "alpha");
        assert_eq!(RhaiEngine::extract_columns("alpha beta gamma", "1", " ", " "), "beta");
        assert_eq!(RhaiEngine::extract_columns("alpha beta gamma", "2", " ", " "), "gamma");
        assert_eq!(RhaiEngine::extract_columns("alpha beta gamma", "-1", " ", " "), "gamma");
    }

    #[test]
    fn test_extract_columns_range() {
        assert_eq!(RhaiEngine::extract_columns("alpha beta gamma delta", "1:3", " ", " "), "beta gamma");
        assert_eq!(RhaiEngine::extract_columns("alpha beta gamma delta", "2:", " ", " "), "gamma delta");
        assert_eq!(RhaiEngine::extract_columns("alpha beta gamma delta", ":2", " ", " "), "alpha beta");
    }

    #[test]
    fn test_extract_columns_multiple() {
        assert_eq!(RhaiEngine::extract_columns("alpha beta gamma delta", "0,2", " ", " "), "alpha gamma");
        assert_eq!(RhaiEngine::extract_columns("alpha beta gamma delta", "1,3", " ", " "), "beta delta");
        assert_eq!(RhaiEngine::extract_columns("alpha beta gamma delta", "0,2,3", " ", " "), "alpha gamma delta");
    }

    #[test]
    fn test_extract_columns_custom_separator() {
        assert_eq!(RhaiEngine::extract_columns("alpha|beta|gamma", "0", "|", " "), "alpha");
        assert_eq!(RhaiEngine::extract_columns("alpha|beta|gamma", "1:3", "|", "|"), "beta|gamma");
        assert_eq!(RhaiEngine::extract_columns("alpha,beta,gamma", "0,2", ",", "-"), "alpha-gamma");
    }

    #[test]
    fn test_extract_columns_out_of_bounds() {
        assert_eq!(RhaiEngine::extract_columns("alpha beta", "5", " ", " "), "");
        
        // This should only extract column 0 since column 5 doesn't exist
        let result = RhaiEngine::extract_columns("alpha beta", "0,5", " ", " ");
        assert_eq!(result, "alpha");
        
        let range_result = RhaiEngine::extract_columns("alpha beta", "5:7", " ", " ");
        assert_eq!(range_result, "");
    }

    #[test]
    fn test_split_text_whitespace() {
        let result = RhaiEngine::split_text("  alpha   beta   gamma  ", " ");
    ...
    fn test_parse_selector_single() {
        assert_eq!(RhaiEngine::parse_selector("0", 3), vec![0]);
        assert_eq!(RhaiEngine::parse_selector("2", 3), vec![2]);
        assert_eq!(RhaiEngine::parse_selector("-1", 3), vec![2]);
    }

    #[test]
    fn test_parse_selector_range() {
        assert_eq!(RhaiEngine::parse_selector("1:3", 5), vec![1, 2]);
        assert_eq!(RhaiEngine::parse_selector("2:", 5), vec![2, 3, 4]);
        assert_eq!(RhaiEngine::parse_selector(":2", 5), vec![0, 1]);
    }

    #[test]
    fn test_parse_selector_multiple() {
        assert_eq!(RhaiEngine::parse_selector("0,2", 5), vec![0, 2]);
        assert_eq!(RhaiEngine::parse_selector("1,3,4", 5), vec![1, 3, 4]);
    }

    #[test]
    fn test_normalize_index() {
        assert_eq!(RhaiEngine::normalize_index(0, 3), Some(0));
        assert_eq!(RhaiEngine::normalize_index(2, 3), Some(2));
        assert_eq!(RhaiEngine::normalize_index(-1, 3), Some(2));
        assert_eq!(RhaiEngine::normalize_index(-3, 3), Some(0));
        assert_eq!(RhaiEngine::normalize_index(3, 3), None);
        assert_eq!(RhaiEngine::normalize_index(-4, 3), None);
    }
    ...
    fn test_col_function_integration() {
        ...
        let result: String = engine.engine.eval_with_scope(&mut scope, r#"line.col("0")"#).unwrap();
        assert_eq!(result, "alpha");
        
        let result: String = engine.engine.eval_with_scope(&mut scope, r#"line.col("1:3")"#).unwrap();
        assert_eq!(result, "beta gamma");
        
        let result: String = engine.engine.eval_with_scope(&mut scope, r#"line.col("0,2")"#).unwrap();
    ...
    fn test_cols_function_integration() {
        ...
        let result: rhai::Array = engine.engine.eval_with_scope(&mut scope, r#"line.cols(["0"])"#).unwrap();
        assert_eq!(result.len(), 1);
        assert_eq!(result[0].clone().into_string().unwrap(), "alpha");
        
        // Test multiple selectors
        let result: rhai::Array = engine.engine.eval_with_scope(&mut scope, r#"line.cols(["0", "2"])"#).unwrap();
        assert_eq!(result.len(), 2);
        assert_eq!(result[0].clone().into_string().unwrap(), "alpha");
        assert_eq!(result[1].clone().into_string().unwrap(), "gamma");
        
        let result: rhai::Array = engine.engine.eval_with_scope(&mut scope, r#"line.cols(["1:3", "-1"])"#).unwrap();
        assert_eq!(result.len(), 2);
