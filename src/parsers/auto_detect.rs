...
fn detect_syslog(line: &str) -> bool {
    ...
    if line.starts_with('<') {
        if let Some(end_bracket) = line.find('>') {
            if end_bracket < 10 { // Reasonable priority field length
                ...
                if after_priority.len() > 3 {
                    ...
                    if matches!(timestamp_part, "Jan" | "Feb" | "Mar" | "Apr" | "May" | "Jun" 
        ...
        if is_likely_ip_address(potential_ip) {
            ...
            if line.contains('[') && line.contains(']') && line.contains(':') {
                ...
                if line.contains("\"GET ") || line.contains("\"POST ") || 
                   ...
                   line.contains("\" ") { // Any quoted request
...
mod tests {
    ...
    fn test_detect_jsonl() {
        assert_eq!(detect_format(r#"{"key": "value", "num": 42}"#).unwrap(), ConfigInputFormat::Jsonl);
        assert_eq!(detect_format(r#"{"timestamp": "2023-04-15T10:00:00Z"}"#).unwrap(), ConfigInputFormat::Jsonl);
    }

    #[test]
    fn test_detect_cef() {
        assert_eq!(detect_format("CEF:0|Vendor|Product|Version|EventID|Name|Severity|Extension").unwrap(), ConfigInputFormat::Cef);
    }

    #[test]
    fn test_detect_syslog() {
        assert_eq!(detect_format("<34>1 2023-04-15T10:00:00.000Z hostname app - - - message").unwrap(), ConfigInputFormat::Syslog);
        assert_eq!(detect_format("<13>Apr 15 10:00:00 hostname program: message").unwrap(), ConfigInputFormat::Syslog);
    }

    #[test]
    fn test_detect_apache() {
        assert_eq!(
            detect_format(r#"192.168.1.1 - - [15/Apr/2023:10:00:00 +0000] "GET /path HTTP/1.1" 200 1234"#).unwrap(),
    ...
    fn test_detect_logfmt() {
        assert_eq!(detect_format("time=2023-04-15T10:00:00Z level=info msg=test").unwrap(), ConfigInputFormat::Logfmt);
        assert_eq!(detect_format("key1=value1 key2=value2 key3=value3").unwrap(), ConfigInputFormat::Logfmt);
    }

    #[test]
    fn test_detect_csv() {
        assert_eq!(detect_format("name,age,city").unwrap(), ConfigInputFormat::Csv);
        assert_eq!(detect_format("1,2,3").unwrap(), ConfigInputFormat::Csvnh);
        assert_eq!(detect_format("john\t25\tnyc").unwrap(), ConfigInputFormat::Tsv); // "john" has letters, so it's treated as header
        assert_eq!(detect_format("name\tage\tcity").unwrap(), ConfigInputFormat::Tsv);
        assert_eq!(detect_format("1\t2\t3").unwrap(), ConfigInputFormat::Tsvnh); // All numeric, no headers
    }

    #[test]
    fn test_detect_line_fallback() {
        assert_eq!(detect_format("just some random text").unwrap(), ConfigInputFormat::Line);
        assert_eq!(detect_format("").unwrap(), ConfigInputFormat::Line);
        assert_eq!(detect_format("a single word").unwrap(), ConfigInputFormat::Line);
    }
}