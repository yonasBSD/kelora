...
#[test]
fn test_keys_emptied_rows_emit_drop_hint() {
    // The quiet data-loss case: `a` is present in some rows but not others, so the
    // rows that lack it become empty after `-k a` and are silently dropped —
    // reported only as "filtered" even though no --filter was given.
    let input = "{\"a\": 1, \"b\": 2}\n{\"b\": 3}\n{\"a\": 4}";

    let (stdout, stderr, exit_code) = run_kelora_with_input(&["-f", "json", "-k", "a"], input);

    assert_eq!(exit_code, 0, "a key-selection drop should remain non-fatal");
    assert_eq!(
        stdout.matches("a=").count(),
        2,
        "only rows with `a` survive"
    );
    assert!(
        stderr.contains("dropped 1 event") && stderr.contains("a"),
        "stderr should explain the silent key-selection drop: {}",
        stderr
    );
}

#[test]
fn test_keys_drop_hint_not_emitted_when_every_row_kept() {
    // Every row carries the selected key, so nothing is emptied — no drop hint.
    let input = "{\"a\": 1}\n{\"a\": 2}";

    let (_stdout, stderr, exit_code) = run_kelora_with_input(&["-f", "json", "-k", "a"], input);

    assert_eq!(exit_code, 0);
    assert!(
        !stderr.contains("dropped"),
        "no rows were emptied, so no drop hint should appear: {}",
        stderr
    );
}

#[test]
fn test_keys_drop_hint_defers_to_typo_hint() {
    // A key that was never present anywhere is a typo, not heterogeneity. The
    // typo hint owns that case; the drop hint must stay quiet to avoid a noisier,
    // less-accurate second line.
    let input = r#"{"level": "INFO"}"#;

    let (_stdout, stderr, exit_code) = run_kelora_with_input(&["-f", "json", "-k", "levle"], input);

    assert_eq!(exit_code, 0);
    assert!(
        stderr.contains("never present"),
        "typo hint should fire: {}",
        stderr
    );
    assert!(
        !stderr.contains("dropped"),
        "drop hint should defer to the typo hint for an unseen key: {}",
        stderr
    );
}

#[test]
fn test_exclude_keys_emptied_rows_emit_drop_hint() {
    // Excluding the only field a row has empties it; the drop should be explained
    // rather than appearing as an unexplained "filtered" count.
    let input = "{\"a\": 1}\n{\"a\": 2, \"b\": 3}";

    let (stdout, stderr, exit_code) =
        run_kelora_with_input(&["-f", "json", "--exclude-keys", "a"], input);

    assert_eq!(exit_code, 0);
    assert!(stdout.contains("b=3"), "the row keeping `b` survives");
    assert!(
        stderr.contains("--exclude-keys dropped 1 event"),
        "stderr should explain the exclude-driven drop: {}",
        stderr
    );
}

#[test]
fn test_keys_drop_hint_suppressed_by_silent() {
    let input = "{\"a\": 1, \"b\": 2}\n{\"b\": 3}";

    let (_stdout, stderr, exit_code) =
        run_kelora_with_input(&["-f", "json", "-k", "a", "--silent"], input);

    assert_eq!(exit_code, 0);
    assert!(
        stderr.trim().is_empty(),
        "--silent should suppress the key-drop hint: {}",
        stderr
    );
}
