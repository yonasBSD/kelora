...
#[test]
fn test_partial_field_skip_does_not_hint() {
    // A field present in some events and absent in others is normal for
    // varying-shape logs — it records a value, so it must NOT trip the typo
    // hint even though some events were skipped. Only a field missing from
    // *every* event (never recorded) is flagged.
    let input = "{\"latency\":10}\n{\"other\":1}\n{\"latency\":20}\n{\"other\":2}\n{\"latency\":30}\n{\"other\":3}\n";
