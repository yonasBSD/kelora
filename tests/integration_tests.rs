...
fn test_parallel_stats_counting_basic() {
    ...
    assert_eq!(output_lines.len(), 10, "Should output exactly 10 lines (multiples of 10)");
    ...
    assert!(stderr.contains("100 total"), "Should show 100 total lines processed");
    assert!(stderr.contains("10 output"), "Should show 10 output lines");
    assert!(stderr.contains("90 filtered"), "Should show 90 filtered lines");
}
...
fn test_parallel_stats_counting_large_dataset() {
    ...
    let input: String = (1..=10000).map(|i| i.to_string()).collect::<Vec<_>>().join("\n");
    
    let (stdout, stderr, exit_code) = run_kelora_with_input(&[
        "--stats",
        "--filter", "line.to_int() % 10 == 0",
        "--parallel",
        "--batch-size", "100"  // Smaller batch size to test multiple batches
    ], &input);
    ...
    // Check statistics in stderr
    assert!(stderr.contains("10000 total"), "Should show 10000 total lines processed");
    assert!(stderr.contains("1000 output"), "Should show 1000 output lines");
    assert!(stderr.contains("9000 filtered"), "Should show 9000 filtered lines");
}

#[test]
fn test_parallel_vs_sequential_stats_consistency() {
    // Test that parallel and sequential modes produce identical statistics
    let input: String = (1..=1000).map(|i| i.to_string()).collect::<Vec<_>>().join("\n");
    
    // Run in sequential mode
    let (stdout_seq, stderr_seq, exit_code_seq) = run_kelora_with_input(&[
        "--stats",
        "--filter", "line.to_int() % 100 == 0"
    ], &input);
    ...
    assert_eq!(stdout_seq, stdout_par, "Sequential and parallel modes should produce identical output");
...
fn test_parallel_stats_with_errors() {
    ...
    let (stdout, stderr, exit_code) = run_kelora_with_input(&[
        "--stats",
        "--filter", "line.to_int() > 3",  // This will cause an error on "invalid"
        "--on-error", "skip",             // Skip errors and continue
        "--parallel"
    ], input);
    ...
    let output_lines: Vec<&str> = stdout.trim().split('\n').collect();
    assert_eq!(output_lines.len(), 2, "Should output 2 lines that pass filter");
    
    // Check statistics - total should be 5, output 2, filtered 3 (including error), errors 0 (when on-error=skip)
    assert!(stderr.contains("5 total"), "Should show 5 total lines processed");
    assert!(stderr.contains("2 output"), "Should show 2 output lines");
    assert!(stderr.contains("3 filtered"), "Should show 3 filtered lines (including error with skip)");
...
fn test_parallel_stats_with_different_batch_sizes() {
    ...
    for &batch_size in &batch_sizes {
        let (stdout, stderr, exit_code) = run_kelora_with_input(&[
            "--stats",
            "--filter", "line.to_int() % 50 == 0",
            "--parallel",
            "--batch-size", &batch_size.to_string()
        ], &input);
        
        assert_eq!(exit_code, 0, "kelora should exit successfully with batch-size {}", batch_size);
    ...
    for (i, (stdout, stderr)) in all_results.iter().enumerate().skip(1) {
        assert_eq!(stdout, first_stdout, "Batch size {} should produce same output as batch size {}", batch_sizes[i], batch_sizes[0]);
        
        // Check that statistics are the same (ignore timing differences)
        let expected_stats = ["500 total", "10 output", "490 filtered"];
        for stat in &expected_stats {
            assert!(first_stderr.contains(stat), "Batch size {} should contain: {}", batch_sizes[0], stat);
            assert!(stderr.contains(stat), "Batch size {} should contain: {}", batch_sizes[i], stat);
        }
    }
}
