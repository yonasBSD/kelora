...
fn test_emit_each_with_base() {
    ...
    let (stdout, stderr, exit_code) = run_kelora_with_input(
        &["-f", "json", "--exec", "let base = #{host: \"server1\", service: \"auth\"}; emit_each(e.users, base)"],
        input,
    );
    ...
    for line in &lines {
        assert!(line.contains("host='server1'"), "Line missing host: {}", line);
        assert!(line.contains("service='auth'"), "Line missing service: {}", line);
    }
...
fn test_emit_each_empty_array() {
    ...
    let (stdout, stderr, exit_code) = run_kelora_with_input(
        &["-f", "json", "--exec", "let count = emit_each(e.empty); e.count = count"],
        input,
    );
    ...
    assert_eq!(stdout.trim(), "", "Expected no output for empty array, got: {}", stdout);
}
...
fn test_emit_each_return_value() {
    ...
    let (stdout, stderr, exit_code) = run_kelora_with_input(
        &["-f", "json", "--exec", "e.count = emit_each(e.items); e.items = ()"],
        input,
    );
...
fn test_emit_each_with_invalid_input() {
    ...
    let (stdout, stderr, exit_code) = run_kelora_with_input(
        &["-f", "json", "--exec", "emit_each(e.not_array)"],
        input,
    );

    assert_eq!(exit_code, 0, "Expected success in resilient mode, stderr: {}", stderr);
...
fn test_emit_each_mixed_array() {
    ...
    let lines: Vec<&str> = stdout.trim().split('\n').collect();
    assert_eq!(lines.len(), 2, "Expected 2 output lines (skipping invalid), got: {}", stdout);
...
fn test_emit_each_with_json_output() {
    ...
    let lines: Vec<&str> = stdout.trim().split('\n').collect();
    assert_eq!(lines.len(), 2, "Expected 2 JSON output lines, got: {}", stdout);
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        assert!(json.get("name").is_some(), "Missing name field in line {}: {}", i, line);
    }
}
...
fn test_emit_each_preserves_line_information() {
    ...
    let (stdout, stderr, exit_code) = run_kelora_with_input(
        &["-f", "json", "--exec", "emit_each(e.events)"],
        input,
    );
...
fn test_emit_each_with_complex_nested_data() {
    ...
    let (stdout, stderr, exit_code) = run_kelora_with_input(
        &["-f", "json", "--exec", "emit_each(e.requests)"],
        input,
    );
...
fn test_emit_each_multi_stage_pipeline() {
    ...
    let (stdout, stderr, exit_code) = run_kelora_with_input(
        &["-f", "json",
          ...
          "--filter", "e.role == \"admin\"",
          "--exec", "e.processed = true; e.stage = \"final\""],
        input,
    );
    ...
    for line in &lines {
        assert!(line.contains("role='admin'"), "Missing admin role: {}", line);
        assert!(line.contains("processed=true"), "Missing processed flag: {}", line);
        assert!(line.contains("stage='final'"), "Missing stage flag: {}", line);
    }
...
fn test_emit_each_nested_pipeline() {
    let input = r#"{"batches": [{"items": [{"id": 1}, {"id": 2}]}, {"items": [{"id": 3}, {"id": 4}]}]}"#;

    let (stdout, stderr, exit_code) = run_kelora_with_input(
        &["-f", "json",
          "--exec", "emit_each(e.batches)",
          "--exec", "emit_each(e.items)",
          "--exec", "e.final_stage = true"],
        input,
    );
    ...
    for line in &lines {
        assert!(line.contains("final_stage=true"), "Missing final_stage: {}", line);
...
fn test_emit_each_with_filter_between_emits() {
    ...
    let lines: Vec<&str> = stdout.trim().split('\n').collect();
    assert_eq!(lines.len(), 2, "Expected 2 items from high-value batch only, got: {}", stdout);
    ...
    for line in &lines {
        assert!(line.contains("stage='final'"), "Missing final stage: {}", line);
    }
}
...
fn test_emit_each_complex_base_and_filtering() {
    ...
    let (stdout, stderr, exit_code) = run_kelora_with_input(
        &["-f", "json",
          ...
          "--exec", "let base = #{source: \"api\", method: e.method}; emit_each(e.logs, base)",
          "--filter", "e.level == \"error\" || e.level == \"warn\"",
          "--exec", "e.severity = \"high\""],
        input,
    );
    ...
    let lines: Vec<&str> = stdout.trim().split('\n').collect();
    assert_eq!(lines.len(), 2, "Expected 2 high-severity logs, got: {}", stdout);

    // Check that we have one error from GET and one warn from POST
    assert!(lines.iter().any(|line|
        line.contains("level='error'") && line.contains("method='GET'")));
    assert!(lines.iter().any(|line|
        line.contains("level='warn'") && line.contains("method='POST'")));

    // All should have base fields and severity
    for line in &lines {
        assert!(line.contains("source='api'"), "Missing source: {}", line);
        assert!(line.contains("severity='high'"), "Missing severity: {}", line);
