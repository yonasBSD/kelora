...
kelora -f syslog /var/log/syslog \
  --filter 'e.severity <= 3' \
  -e 'let ip = e.message.extract_ip(); if ip != "" { track_freq("ip", ip) }' \
  --metrics
```

- `track_freq("hostname", e.hostname)` surfaces noisy machines.
