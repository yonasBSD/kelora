...
### Level
The severity or importance of a log event. Common levels: DEBUG, INFO, WARN, ERROR, CRITICAL. Can be filtered with `-l` (include) or `-L` (exclude). Case-insensitive. See [Basics Tutorial](tutorials/basics.md#level-filtering-l-l).
...
### Rhai
The embedded scripting language used for filters and transforms. Rust-based with JavaScript-like syntax. Provides 40+ built-in functions for log analysis. See [Introduction to Rhai Tutorial](tutorials/intro-to-rhai.md) and [Rhai Cheatsheet](reference/rhai-cheatsheet.md).
...
e["field-with-dashes"]    // Bracket notation
e.nested.field            // Nested access
e.get_path("a.b.c", "")   // Safe nested access with default
```

### Type Conversion
```rhai
e.status.to_int()         // Convert to integer (fails on error)
e.status.to_int_or(0)     // Convert with fallback
e.count.to_float_or(0.0)  // Convert to float with fallback
e.value.to_string()       // Convert to string
```

### Filtering
```rhai
e.level == "ERROR"                     // String equality
e.status >= 500                        // Numeric comparison
e.level in ["ERROR", "CRITICAL"]       // Membership test
e.message.contains("timeout")          // String search
e.has("optional_field")                // Field existence
```

### Common CLI Patterns
```bash
# Read JSON, filter errors, show essentials
kelora -j app.log -l error -c

# Parse custom format, extract data, export CSV
kelora -f 'cols:ts level *msg' app.log -e 'e.absorb_kv("msg")' -F csv

# Parallel processing of compressed archives
kelora -j logs/*.gz --parallel -l error

# Track metrics across events
kelora -j app.log -e 'track_count(e.service)' --metrics

# Time-based filtering
kelora -j app.log --since 1h --until 10m
```
...
- [Function Reference](reference/functions.md) - All 40+ built-in functions
