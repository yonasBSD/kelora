...
alerts = --levels critical,error --exec 'track_freq("service", e.service)'
```
...
track_freq("service_level", e.service + ":" + e.level);
...
analyze = -a enrich --exec 'track_freq("severity", e.severity)' --metrics
```
...
traffic = --exec 'track_freq("method", e.method); track_freq("status", e.status)' --metrics
```
</details>
