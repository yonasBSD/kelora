...
kelora -j app.log --resilient \
    ...
    -e 'track_freq("status", e.risky.status)' \     # Only runs if risky succeeded
    -k safe_field,risky
```
...
kelora -j app.log \
    ...
    --exec 'track_freq("endpoint", e.endpoint)'       # Track them
```
