...
- **Multiline**: Start with presets like `-M stacktrace`, `-M docker`, `-M syslog`, `-M combined`, `-M nginx`, or `-M continuation`; run `kelora --help-multiline` for advanced recipes and custom patterns
...
| `combined` | Apache/Nginx access logs (`127.0.0.1 - - [date] ...`) | `-M start:^\S+\s+\S+\s+\S+\s+\[` |
