...
> [!NOTE]
> Pre-1.0 tool generated entirely by AI agents. Validated by a large automated test suite and Rust security tools; expect breaking changes before 1.0. See [Development Approach](#development-approach) and [SECURITY.md](SECURITY.md) before relying on it in production.
...
Kelora is an experiment in agentic AI development. Claude Code (Sonnet 4/4.5) and Codex CLI (`gpt-5-codex`) generate all implementation and tests; I guide requirements but do not write or review code. Validation relies on the automated test suite plus `cargo audit` and `cargo deny`, so please inspect the code yourself before relying on it in production and expect the API to evolve until 1.0. This is a spare-time solo project, so responses and updates happen on a best-effort basis.
