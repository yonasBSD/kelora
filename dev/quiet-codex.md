...
- `--silent`: suppress everything except exit code: events, diagnostics, stats, metrics (stderr and file), script output. Also sets Rhai side effect suppression (prints, fs warnings). **Dominates all emitter flags and config defaults**—no conflicts; any emitters requested are simply ignored.
- `--no-silent`: disable a silent default from config; restores normal output unless suppressed by other flags (e.g., `-q`).
- `--metrics-only`: suppress events and diagnostics, suppress stats; emit metrics (stderr/JSON/file) unless `--silent` is present. Implies `-F none`, `--no-diagnostics`, `--no-stats`; does **not** imply `--silent`. On fatal errors, emit exactly one concise diagnostic line to stderr and exit non-zero (matches current `--stats-only`).
- No conflicts: `--silent` overrides/ignores all emitters (`--metrics*`, `--metrics-only`, `--stats`) from config or CLI. Suppress-only flags are allowed but redundant.
...
- `--metrics-only`: no events, no diagnostics (except single fatal line), no stats; metrics emit (stderr/JSON/file) unless `--silent` present (suppressed silently).
...
- Metrics/stats: `--silent` suppresses metrics (table/JSON) and prevents metrics file writes; suppresses stats/error summaries. `-q` does not impact metrics/stats; no conflicts.
...
- `--metrics-only` interaction: combining with `--silent` is allowed; emitters are suppressed. Combining with `--metrics/--metrics-file/--metrics-json` is redundant but allowed.
- Silent dominance: `--silent` may be combined with any emitter flag or config default; it wins and suppresses all output/writes. Allow `--silent` with suppress-only flags (`-q`, `--no-events`, `--no-diagnostics`, `--no-script-output`).
...
- CLI: repurpose `-q/--quiet` help text to "suppress events"; keep `--no-events` alias; add `--no-diagnostics`; add `--no-silent` to flip config defaults off; redefine `--silent` help to promise zero output; optional `--no-script-output` gate. No conflicts when combining `--silent` with emitters—silent dominates.
