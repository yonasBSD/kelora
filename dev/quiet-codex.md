...
- `--no-diagnostics`: suppress diagnostics and error summaries; still permit the single fatal line; does not affect events.
...
- `--silent`: no stdout/stderr (events/diagnostics/stats/terminal metrics/script output) except the single fatal line; metrics files still written.
...
- Error summary path: automatic summaries/stats fallbacks should honor `--silent` and `--no-diagnostics` (not tied to quiet ladder anymore) while still allowing the single fatal line.
...
- CLI: repurpose `-q/--quiet` help text to "suppress events"; keep `--no-events` alias; add `--no-diagnostics`; add `--no-silent` to flip config defaults off; redefine `--silent` help to promise no stdout/stderr except the single fatal line while allowing metrics files; optional `--no-script-output` gate. No conflicts when combining `--silent` with emitters—silent dominates terminal sinks.
- Config derivation: swap quiet level ladder for booleans: `quiet_events` (from -q/--no-events, `-F none`, or `-S/--stats-only`), `suppress_diagnostics`, `silent`, `suppress_script_output` (silent or explicit). Keep `OutputFormat::None` respected independently but OR-ed with quiet for events; `-S/--stats-only` should force `OutputFormat::None` in addition to quieting events.
