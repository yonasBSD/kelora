...
- `-v` (progress + errors):
  ...
  - Progress blocks: first block after whichever comes later: 5s or 5000 processed events since start; subsequent blocks keyed off last tick using the same rule. Skip if nothing processed. Counts: lines read, events emitted, errors by type (top 3), spans open/closed, throughput estimate (cumulative evts/sec).
- `-vv` (sampled flow):
  - Includes `-v`.
...
- Global trace cap per process: `MAX_TRACE_LINES=200` across sampled traces plus Rhai tracer; notice counts toward cap.
- Truncation: script/expr previews 80 chars; key diffs 8 keys; formatter preview 120 chars; UTF-8 safe with `...` suffix. No full payloads before `-vvv`.
...
- Event IDs assigned by coordinator, global monotonic; parallel traces include worker tag (e.g., `w2:e1043`) but sampling uses the global ID for determinism.
- Per-event traces: workers decide sampling deterministically from event ID; send trace lines through the existing ordered stderr capture with a bounded buffer (500 trace lines per worker). If buffer full, workers drop silently; coordinator prints a single “trace cap reached” notice on first drop.
...
- Honor `--silent`/`--metrics-only`/`--stats-only`/`--no-events`/`--quiet` → suppress traces/progress; still allow fatal errors.
